...
layout: page-sidenav
group: "Chapter. 2"
title: "2.3. The Gaussian Distribution [II]"
---
...
$$\ln p({\bf X}|{\bf \mu}, \Sigma) = -\frac{ND}{2}\ln(2\pi) - \frac{N}{2}\ln|\Sigma|-\frac{1}{2}\sum_{n=1}^{N}({\bf x}_n-{\bf \mu})^T\Sigma^{-1}({\bf x}_n-{\bf \mu}) \qquad{(2.118)}$$
...
$$\dfrac{\partial}{\partial {\bf \mu}}\ln p({\bf X}|{\bf \mu}, \Sigma) = \sum_{n=1}^{N}\Sigma^{-1}({\bf x}_n-{\bf \mu}) \qquad{(2.120)}$$

- 이 값이 0이 되는 지점에서 최대값을 가지게 된다.
- 따라서 이 때의 평균 값은 다음과 같게 된다.

$${\bf \mu}_{ML} = \frac{1}{N}\sum_{n=1}^{N}{\bf x}_n \qquad{(2.121)}$$

- 공분산도 별거 없다. 계산의 편리성을 위해 \\( \Lambda=\Sigma^{-1} \\) 로 놓고 풀면 된다.

$$\Sigma_{ML} = \frac{1}{N}\sum_{n=1}{N}({\bf x}-{\bf \mu}_{ML})({\bf x}-{\bf \mu}_{ML})^T \qquad{(2.122)}$$ 

- 이 식에서는 \\( {\bf \mu}\_{ML} \\) 이 사용된다. 먼저 \\( {\bf \mu}\_{ML} \\) 을 구하고 \\( \Sigma\_{ML} \\) 을 구하면 된다.

- 이 때의 평균 값은 다음과 같다.

$$E[{\bf \mu}_{ML}] = {\bf \mu} \qquad{(2.123)}$$
...
$$\tilde{\Sigma} = \frac{1}{N-1}\sum_{n=1}^{N}({\bf x}_n - {\bf \mu}_{ML})({\bf x}_n - {\bf \mu}_{ML})^T \qquad{(2.125)}$$
...
$$\ln p({\bf X}|{\bf \mu}, \Sigma) = -\frac{ND}{2}\ln(2\pi) - \frac{N}{2}\ln|\Sigma|-\frac{1}{2}\sum_{n=1}^{N}({\bf x}_n-{\bf \mu})^T\Sigma^{-1}({\bf x}_n-{\bf \mu})$$
...
- \\( {\bf y}=({\bf x}-{\bf \mu}) \\) 라 하면 다음의 식이 유도된다.

$$\frac{\partial}{\partial {\bf \mu}}({\bf x}-{\bf \mu})^T\Sigma^{-1}({\bf x}-{\bf \mu}) = \frac{\partial}{\partial {\bf y}}{\bf y}^T\Sigma^{-1}{\bf y} \frac{\partial {\bf y}}{\partial {\bf \mu}} = -(\Sigma^{-1}+(\Sigma^{-1})^{T}){\bf y} \equiv -(\Lambda-\Lambda^T){\bf y}$$

- 따라서

$$\frac{\partial}{\partial {\bf \mu}}l({\bf \mu}, \Sigma) = -\frac{1}{2}\sum_{i=1}^{N}-2\Lambda({\bf x}_i-{\bf \mu})=\Lambda\sum_{i=1}^{N}({\bf x}_i-{\bf \mu}) = 0$$

- 참고로 공분산은 대칭행렬이라서 위와 같이 전개된다. ( \\( \Sigma^{-1}=(\Sigma^{-1})^T \\))
- 이제 *MLE* 평균은 다음과 같아진다.

$${\bf \mu}_{ML} = \frac{1}{N}\sum_{i=1}^{N}{\bf x}_i = \bar{\bf x}$$

*Covariance*

- 참고로 \\( \Lambda \equiv \Sigma^{-1} \\) 이고 \\( {\vert}\Lambda{\vert} = {\vert}\Sigma{\vert} \\) 이다.

$$l(\Lambda) = \frac{N}{2}\ln|\Lambda| - \frac{1}{2}\sum_i tr[({\bf x}_i-{\bf \mu})({\bf x}_i-{\bf \mu})^T\Lambda] =  \frac{N}{2}\ln|\Lambda| - \frac{1}{2} tr[S_{\mu}\Lambda]$$

$$S_{\mu} = \sum_{i=1}^{N}({\bf x}_i-{\bf \mu})({\bf x}_i-{\bf \mu})^T$$
...
- *MLE* 로 얻어진 \\( {\bf \mu}_{ML} \\) 식을 업데이트 방식으로 바꾸어보자.
    - \\( {\bf \mu}_{ML} \\) 에서 마지막 샘플을 추출해보자.
  
$${\bf \mu}_{ML}^{(N)} = \frac{1}{N} \sum_{n=1}^{N}{\bf x}_n = \frac{1}{N}{\bf x}_N + \frac{1}{N}\sum_{n=1}^{N-1}{\bf x}_n\\
= \frac{1}{N}{\bf x}_N + \frac{N-1}{N}{\bf \mu}_{ML}^{(N-1)}={\bf \mu}_{ML}^{(N-1)}+\frac{1}{N}({\bf x}_N-{\bf \mu}_{ML}^{(N-1)}) \qquad{(2.126)}$$

- \\( N-1 \\) 개의 데이터로부터 추정된 \\( {\bf \mu}\_{ML}^{(N-1)} \\) 와 \\( N \\) 번째 관측된 데이터를 이용하여 \\( {\bf \mu}\_{ML}^{(N)} \\) 을 구한다.
...
- 만약 1차원이 아니라 \\( D \\) 차원일 때에는 어떻게 처리해야 하나?
    - 사실 별로 어렵지는 않다.
    - 다변량 가우시안 분포 \\( N({\bf x}\|{\bf \mu}, \Lambda^{-1}) \\) 을 도입하여 전개하면 된다.
...
$$p({\bf \mu}, \Lambda|\mu_0, \beta, {\bf W}, v) = N({\bf \mu}|{\bf \mu}_0, (\beta\Lambda)^{-1})W(\Lambda|{\bf W}, v)$$
